...
class FES_Flat_Rate_Shipping_Field extends FES_Field {
	...
	public $supports = array(
		...
		'template'	  => 'edd_flat_rate_shipping',
	...
	public function set_title() {
		$title = _x( 'Shipping', 'FES Field title translation', 'edd-flat-rate-shipping' );
		$title = apply_filters( 'fes_' . $this->name() . '_field_title', $title );
		$this->supports['title'] = $title;
	}

	public function exclude_from_admin( $fields ){
		array_push( $fields, 'edd_flat_rate_shipping' );
		return $fields;
	}
	...
	public function render_field_frontend( $user_id = -2, $readonly = -2 ) {
		...
		$user_id   = apply_filters( 'fes_render_edd_flat_rate_shipping_field_user_id_frontend', $user_id, $this->id );
		$readonly  = apply_filters( 'fes_render_edd_flat_rate_shipping_field_readonly_frontend', $readonly, $user_id, $this->id );
		...
		<div class="fes-fields <?php echo sanitize_key( $this->name()); ?>">
			...
			<div class="edd-fes-shipping-fields">
				...
				<input class="edd-shipping-field textfield<?php echo esc_attr( $required ); ?>" id="edd_flat_rate_shipping[domestic]" type="text" data-required="<?php echo $required ?>" data-type="text" name="<?php echo esc_attr( $this->name() ); ?>[domestic]" placeholder="<?php echo __( 'Enter the domestic shipping charge amount', 'edd-flat-rate-shipping' ); ?>" value="<?php echo esc_attr( $domestic ) ?>" size="10" <?php $this->required_html5( $readonly ); ?> <?php echo $readonly ? 'disabled' : ''; ?> />
				<input class="edd-shipping-field textfield<?php echo esc_attr( $required ); ?>" id="edd_flat_rate_shipping[international]" type="text" data-required="<?php echo $required ?>" data-type="text" name="<?php echo esc_attr( $this->name() ); ?>[international]" placeholder="<?php echo __( 'Enter the international shipping charge amount', 'edd-flat-rate-shipping' ); ?>" value="<?php echo esc_attr( $international ) ?>" size="10" <?php $this->required_html5( $readonly ); ?> <?php echo $readonly ? 'disabled' : ''; ?> />
			</div>
		</div> <!-- .fes-fields -->
        <?php
		$output .= ob_get_clean();
	...
	public function validate( $values = array(), $save_id = -2, $user_id = -2 ) {
		...
		if ( $this->required() ){
			...
			if ( ! isset( $values[ $name ]['domestic'] ) ) {
				return __( 'Please enter your domestic shipping price.', 'edd-flat-rate-shipping' );
			}
			if ( ! isset( $values[ $name ]['international'] ) ) {
				return __( 'Please enter your international shipping price.', 'edd-flat-rate-shipping' );
			}
		} else {
